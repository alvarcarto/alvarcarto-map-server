...
# Tile service
status_code=$(curl --silent --output /dev/null --write-out "%{http_code}" 'http://localhost:8002/bw/13/4665/2367/tile.png')
...
# Placement service
status_code=$(curl --silent --output /dev/null --write-out "%{http_code}" 'http://localhost:8003/api/place-map/no-flowers-in-blue-black-frame?swLat=60.04&swLng=24.73&neLat=60.37&neLng=25.20&mapStyle=bw&posterStyle=sharp&labelsEnabled=true&labelHeader=Helsinki&labelSmallHeader=Finland&labelText=60.209°N%20%2F%2024.973°E&resizeToWidth=400')
if [ $status_code -ne 200 ]; then
...
# http-cache service
status_code=$(curl --silent --output /dev/null --write-out "%{http_code}" 'http://localhost:8004/placement/api/place-map/no-flowers-in-blue-black-frame?swLat=60.04&swLng=24.73&neLat=60.37&neLng=25.20&mapStyle=bw&posterStyle=sharp&labelsEnabled=true&labelHeader=Helsinki&labelSmallHeader=Finland&labelText=60.209°N%20%2F%2024.973°E&resizeToWidth=400')
if [ $status_code -ne 200 ]; then
